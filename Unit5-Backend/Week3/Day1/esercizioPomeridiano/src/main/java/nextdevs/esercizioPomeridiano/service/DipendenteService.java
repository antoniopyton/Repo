...
import nextdevs.esercizioPomeridiano.repository.DipendenteRepository;
...
public class DipendenteService {
    ...
    public String saveDipendente(DipendenteDto dipendenteDto) {
        ...
        return "Dipendente salvato con successo con id: " + dipendente.getId();
    }
    ...
    public Dipendente getDipendenteByEmail(String email) throws DipendenteNonTrovatoException {
        Optional<Dipendente> dipendenteOptional = dipendenteRepository.findByEmail(email);

        if (dipendenteOptional.isPresent()) {
            return dipendenteOptional.get();
        } else {
            throw new DipendenteNonTrovatoException("User with email " + email + " does not exist");
        }
