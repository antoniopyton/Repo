...
// let random2 = Math.floor(Math.random() * 110) +1;
// console.log(random2);

// let random3 = Math.floor(Math.random() *50) +1;
// console.log(random3);

// let random4 = Math.floor(Math.random() *500) +1;
// console.log(random4);
...
splitMe = (stringa) => {
    ...
    splitMe("Ciao, mi chiamo Antonio, tu?");
...
console.log(deleteOne("cammello", false));
...
onlyLetters = (string) => {
  ...
  console.log(onlyLetters("H3llo world"));
...
console.log(isThisAnEmail("acopetti99@gmail.com"));
...
whatDayIsIt = () => {
  ...
  let index = giornoCorrente.getDay();
  return giorniDellaSettimana[index];
...
//     var risultatoSingoloDado = dice();
//     risultati.sum += risultatoSingoloDado;
//     risultati.values.push(risultatoSingoloDado);
//   }
...
onlyInLastMillennium = (arrayMovies) => {
...
searchByTitle = (stringa) => {
  let trovaParola = movies.filter(function(movies) {
...
const removeIndex = (numero) => {
  const nuovoArray = [...movies];
  nuovoArray.splice(numero, 1);
  return nuovoArray;
};
